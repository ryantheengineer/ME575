...
gradf0 = gradient_f(x0)
gradg0 = gradient_g(x0)

LagrHess0 = eye(2);

Jacobian0 = [transpose(gradg0)]

% Assemble the coefficient matrix:
coeffMat0 = [LagrHess0(1,1),LagrHess0(1,2),0,-Jacobian0(1);
            LagrHess0(2,1),LagrHess0(2,2),0,-Jacobian0(2);
            0, 0, lamda0, s0;
            Jacobian0(1),Jacobian0(2), -1, 0]
...
% evec = [1;1];
resid0 = zeros(4,1);
resid0(1) = gradf0(1) - lamda0*gradg0(1);
resid0(2) = gradf0(2) - lamda0*gradg0(2);
resid0(3) = s0*lamda0 - mu0;
resid0(4) = g0 - s0;
resid0


% Solve for the delta vector using coeffMat and resid2
delta_vec = coeffMat0\(-resid0)
...
% P = f3 + 2*abs(g3)
% 
% % The merit function increases, so we cut back our step by 0.5
% x3 = x2 + 0.5*delta_vec(1:2)
% s3 = s2 + 0.5*delta_vec(3)
% lamda3 = lamda + 0.5*delta_vec(4)
% 
% f3 = objective(x3)
% g3 = constraint(x3)
% 
% P = f3 + abs(lamda3)*abs(g3)
% 
% % Cut back by another 0.5 to get lower merit function
% x3 = x2 + 0.25*delta_vec(1:2)
% s3 = s2 + 0.25*delta_vec(3)
% lamda3 = lamda + 0.25*delta_vec(4)
% 
% f3 = objective(x3)
% g3 = constraint(x3)
% 
...
% 
% % Cut back by another 0.5 to get lower merit function
% x3 = x2 + 0.125*delta_vec(1:2)
% s3 = s2 + 0.125*delta_vec(3)
% lamda3 = lamda + 0.125*delta_vec(4)
% 
% f3 = objective(x3)
% g3 = constraint(x3)
% 
